...
| 算法 | 方法名 | 词性过滤 | 适用场景 |
|------|--------|----------|----------|
| **TF-IDF** | `extract_keywords()` | 名词、动词、英文 | 通用、快速、效果好 |
| **TextRank** | `extract_keywords_textrank()` | 仅名词类 | 长文本、学术文本 |
...
**停词表**：内置中英文停词及LaTeX常见符号
- 中文：的、是、在、了、和、等
- 英文：the、a、is、was、have等
- LaTeX符号：frac、sqrt、alpha、beta等
...
│   │   ├── json_mixin.py            # JSON字段处理mixin
│   │   ├── note.py                  # Note - 笔记模型
│   │   ├── card.py                  # ReviewCard, ReviewLog模型
│   │   ├── category.py              # Category - 分类模型
│   │   └── settings.py              # Settings - 系统配置模型
...
#### SettingsDAO
```python
get(key) -> Settings               # 获取配置项
get_all() -> List[Settings]        # 获取所有配置
set(key, value) -> Settings        # 设置配置值
set_bool(key, value) -> Settings   # 设置布尔配置
init_defaults()                    # 初始化默认配置
```
...
**词性过滤**：`n, nr, ns, nt, nz, v, vn, eng`（名词、动词、英文）
...
**实现**：基于jieba的TextRank提取

**词性过滤**：`n, nr, ns, nt, nz, eng`（仅名词类和英文）
...
**初始化**：
$$I(1) = 1 \text{ day}, I(2) = 6 \text{ days}, E = 2.5$$
